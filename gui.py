...
# Глобальные переменные (временно)
sim_size = 125
layer = ''
...
class MainWindow(QWidget):
    def __init__(self):
        ...
        self.size_edit = QLineEdit(str(sim_size))
    ...
    def start_simulation(self):
        global sim_size, layer  # Используем глобальные переменные (временно)
        sim_size = int(self.size_edit.text())
        layer = self.layer_combo.currentText()

        # Запуск вашей симуляции здесь
        # ... (код из quantumwaves.py для инициализации и запуска симуляции)
        do_parallel = False
        do_collapse = False
        sim_size = 125
